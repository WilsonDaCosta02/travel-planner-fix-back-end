...
const resolvers = {
  Query: {
    ...
    trips: async (_, { user_id }) => {
      const [rows] = await db.query('SELECT * FROM trip WHERE user_id = ? ORDER BY start_date', [user_id]);
      return rows;
    },

    dreamDestinations: async (_, { user_id }) => {
      const [rows] = await db.query('SELECT * FROM dream_trip WHERE user_id = ?', [user_id]);
  ...
  Mutation: {
    createUser: async (_, { nama, no_hp, email, password }) => {
      ...
      const [result] = await db.query('INSERT INTO user (nama, no_hp, email, password) VALUES (?, ?, ?, ?)', [nama, no_hp, email, hashedPassword]);
    ...
    updateUser: async (_, { id, nama, no_hp, email, password, foto }) => {
      ...
      await db.query('UPDATE user SET nama = ?, no_hp = ?, email = ?, password = ?, foto = ? WHERE id = ?', [newNama, newNoHp, newEmail, newPassword, newFoto, id]);
    ...
    createTrip: async (_, { user_id, title, location, remarks, start_date, end_date }) => {
      const [result] = await db.query('INSERT INTO trip (user_id, title, location, remarks, start_date, end_date) VALUES (?, ?, ?, ?, ?, ?)', [user_id, title, location, remarks, start_date, end_date]);
    ...
    updateTrip: async (_, { id, title, location, remarks, start_date, end_date }) => {
      ...
      await db.query('UPDATE trip SET title = ?, location = ?, remarks = ?, start_date = ?, end_date = ? WHERE id = ?', [newTitle, newLocation, newRemarks, newStartDate, newEndDate, id]);
    ...
    createDreamDestination: async (_, { user_id, name, image }) => {
      const [result] = await db.query('INSERT INTO dream_trip (user_id, name, image) VALUES (?, ?, ?)', [user_id, name, image]);
    ...
    updateDreamDestination: async (_, { id, name, image }) => {
      ...
      await db.query('UPDATE dream_trip SET name = ?, image = ? WHERE id = ?', [newName, newImage, id]);
    ...
    triggerTestNotifH3: async () => {
      ...
      try {
        const [rows] = await db.query(
          `
          SELECT t.*, u.id AS user_id, u.nama 
          FROM trip t
        ...
        rows.forEach((trip) => {
          const message = ` Hi ${trip.nama}, trip "${trip.location}" berangkat dalam 3 hari!`;
          sendNotificationToUser(trip.user_id, message);
          console.log(`📤 Dikirim ke user ${trip.user_id}`);
        ...
        console.error('❌ Error trigger notif H-3:', error.message);
        throw new Error('Gagal trigger notifikasi H-3');
      }
    },
  },
